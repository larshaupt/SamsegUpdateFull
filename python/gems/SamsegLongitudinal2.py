...
class SstModel:
    ...
    def __init__(self, 
        ...
        # write all parameters from above into class variables
        self.sstFileNames = sstFileNames
    ...
    def preprocessSubjectSpecificTemplate(self):
    ...
    def fitSubjectSpecificTemplate(self):
        self.sstModel.fit()


    def getSubjectSpecificTemplate(self):
        return self.sstModel
...
class SamsegLongitudinal2:
    def __init__(self,
        ...
        self.imageWeights = imageWeights
        ...
        # New code
        if len(self.imageWeights) != self.numberOfTimepoints:
            print(f"Did not provide the correct amount of weights: {self.imageWeights}. It should have the length {len(self.imageFileNamesList)}")
            self.imageWeights = [1]*self.numberOfTimepoints

        if not all([isinstance(el, float) or isinstance(el,int) for el in self.imageWeights]):
            if not all([el.replace(".", "").isnumeric() for el in self.imageWeights]):
                print(f"Did not provide the correct weights: {self.imageWeights}. It should be floats")
                self.imageWeights = [1]*self.numberOfTimepoints
            else:
                self.imageWeights = [float(el) for el in self.imageWeights]
    ...
    def fitModel(self):
        ...
        while True:
            ...
            self.latentAtlasFileName = os.path.join(latentAtlasDirectory, 'latentAtlas_iteration_%02d.mgz' % (iterationNumber + 1))
            self.probabilisticAtlas.saveDeformedAtlas(self.latentDeformationAtlasFileName, self.latentAtlasFileName, self.latentDeformation, True)
            ...
            for timepointNumber in range(self.numberOfTimepoints):
                ...
                self.timepointModels[timepointNumber].optimizationOptions['multiResolutionSpecification'][0]['atlasFileName'] = self.latentAtlasFileName
            ...
            if self.hiddenState != None:
                hiddenStateMesh = self.probabilisticAtlas.getMesh(self.hiddenState, transformUsedForEstimation)
                timepointPositions.append(hiddenStateMesh.points)
            mesh_collection.set_positions(referencePosition, timepointPositions)

            # Read mesh in sst warp
            mesh = self.probabilisticAtlas.getMesh(self.latentAtlasFileName, transformUsedForEstimation)
