...
def matchTimePointNames(t1, t2):
    assert len(t1) == len(t2)
    return all([os.path.basename(m1)==os.path.basename(m2) for (m1,m2) in zip(t1,t2)])
...
class SamsegLongitudinalUpdateWeighted:
    def __init__(self,
        imageFileNamesList,
        imageTimePoints,
        updateFileNamesList,
        ...
        self.updateFileNamesList = updateFileNamesList
        self.imageTimePoints = imageTimePoints #TODO: Chaneg this later back
        ...
        self.timepointNumberProcessed = []
        self.timepointNumberUpdate = list(range(self.numberOfTimepoints))
        self.latentInitialized = False
        self.imageToImageTransformMatrix = None
        # New code
        ...
        def sigmoid_kernel(a,b):
            x = a-b
            return 1/(np.exp(x*self.RBFLength ) + 2 + np.exp(-x*self.RBFLength ))*self.RBFScale + self.RBFintercept
        self.weightKernel = sigmoid_kernel
        print("Kernel parameters: ",self.RBFLength, self.RBFScale, self.RBFintercept, "sigmoid", self.n_neighbors)


        

        if not all([el in self.imageFileNamesList for el in self.updateFileNamesList]):
            print(f"\n ############# WARNING ############## \n Not all files from updateFiles are included in imageFiles. Please specify the correct file names \n")
            self.imageFileNamesList.extend([el for el in self.updateFileNamesList if el not in self.imageFileNamesList])

        # All files that are in imageFileNamesList but not in updateFileNamesList

        self.processedFileNamesList = [el for el in self.imageFileNamesList if not any([matchTimePointNames(el, up) for up in self.updateFileNamesList])]
       
        if len(self.processedFileNamesList) == 0:
            self.runWithPreviousState = False
        else:
            self.runWithPreviousState = True

    def loadFinalState(self):
            
        finalStateDir = os.path.join(self.savePath, 'finalState.p')
        if not os.path.exists(finalStateDir):
            print(f"\n###### WARNING #######\nFinal State File not found. Updating the files without prior states.\n")
            self.processedFileNamesList = []
            self.runWithPreviousState = False
            return None
        with open(finalStateDir, "rb") as infile:
            self.finalState = pickle.load(infile)
        

    
    def loadProcessedFiles(self):

        if self.finalState == None:
            self.loadFinalState()

        # What do we need to load?
        # All elements from self.processedFileNamesList
        #
        ...
        # self.sstModel.biasField.getBiasFields( self.sstModel.mask)
        # self.sstModel.mask
        ...
        self.sstModel.initializeGMM()
        self.sstModel.initializeBiasField()
        self.sstModel.gmm.means = self.finalState["sstMeans"]
        self.sstModel.gmm.variances = self.finalState["sstVariances"]
        self.sstModel.gmm.mixtureWeights = self.finalState["sstMixtureWeights"]
        self.sstModel.biasField.coefficients = self.finalState["sstBiasFieldCoefficients"]
        self.sstModel.deformation = self.finalState["sstDeformation"]
        self.sstModel.deformationAtlasFileName = self.finalState["sstDeformationAtlasFileName"]
        self.sstModel.optimizationSummary = self.finalState["sstOptimizationSummary"]
        self.sstModel.optimizationHistory = self.finalState["sstOptimizationHistory"]
        self.sstModel.modelSpecifications.FreeSurferLabels = self.finalState["labels"]
        self.sstModel.modelSpecifications.names = self.finalState["names"]
        self.sstModel.numberOfGaussiansPerClass = self.finalState["sstNumberOfGaussiansPerClass"]
        self.timepointVolumesInCubicMm = self.finalState["timepointVolumesInCubicMm"]
        self.optimizationSummary = self.finalState["optimizationSummary"]
        #self.imageToImageTransformMatrix = self.finalState["imageToImageTransformMatrix"]
        
        # self.latentMeans = self.finalState["latentMeans"]
        # self.latentVariances = self.finalState["latentVariances"]
        # self.latentMixtureWeights = self.finalState["latentMixtureWeights"]
        # self.latentDeformation = self.finalState["latentDeformation"]
        # self.latentDeformationAtlasFileName = self.finalState["latentDeformationAtlasFileName"]
        self.timepointNumberProcessed = []
        for timepointModel in self.finalState["timepointModel"]:
            # find the matching timpoints
            timepointNames = timepointModel["timepointFileName"]
            timepointNumbers = [i for i in range(self.numberOfTimepoints) if matchTimePointNames(self.imageFileNamesList[i], timepointNames)]
            ...
            if len(timepointNumbers) == 0:
                continue
            if not any([matchTimePointNames(el, timepointNames) for el in self.processedFileNamesList]):
                print(f"\n ############# WARNING ############## \n Could not find matching saved segmentation for {timepointNames}. Skipping \n")

            timepointNumber = timepointNumbers[0]

            self.timepointNumberProcessed.append(timepointNumber)

            self.timepointModels[timepointNumber].initializeGMM()
            self.timepointModels[timepointNumber].gmm.means = timepointModel["timepointMeans"]
            self.timepointModels[timepointNumber].gmm.variances = timepointModel["timepointVariances"]
            self.timepointModels[timepointNumber].gmm.mixtureWeights = timepointModel["timepointMixtureWeights"]
            #self.timepointModels[timepointNumber].biasField.coefficients = timepointModel["timepointBiasFieldCoefficients"]
            # this one is the deformation from the template space. It gets changed at postprocessing. Hence, this should be saved before postprocessing
            self.timepointModels[timepointNumber].deformation = timepointModel["timepointDeformations"]# - self.finalState["latentDeformation"]
            self.timepointModels[timepointNumber].deformationAtlasFileName = timepointModel["timepointDeformationAtlasFileNames"]
            print(f"Loaded timepoint {timepointNames}")

            # account for this in postprocessing
    ...
    def segment(self, saveWarp=False):
        ...
        self.timer = Timer()
        if self.runWithPreviousState:
            self.loadFinalState()
        self.constructAndRegisterSubjectSpecificTemplate()
        self.timer("constructAndRegisterSubjectSpecificTemplate")
        # construct sstModel
        
        self.preProcess()
        self.timer("preProcess")
        if self.runWithPreviousState:
            self.loadProcessedFiles()
            self.timer("loadProcessedFiles")
        self.fitModel()
        self.timer("fitModel")
        self.postProcess(saveWarp=saveWarp)
        self.timer("postProcess")
        return None
    ...
    def constructAndRegisterSubjectSpecificTemplate(self):
        ...
        if len(self.processedFileNamesList) == 0:
            # create new SubjectSpecificTemplate
            print("Creating new SubjectSpecificTemplate")
            ...
            # resue previous SubjectSpecificTemplate
            print("Loading SubjectSpecificTemplate")
            self.sstFileNames = self.loadSubjectSpecificTemplate()

            self.imageToImageTransformMatrix = self.finalState["imageToImageTransformMatrix"]
        
        # self.imageToImageTransformMatrix should be constant over all runs. Hence, we either need to save it or compute it always from the same files
        # It should be constant since we use the same sst Template and register it to the same Atlas
    ...
    def fitModel(self):
        ...
        #pdb.set_trace()
        for timepointNumber in self.timepointNumberUpdate:
            self.timepointModels[timepointNumber].initializeGMM()
            ...
            self.latentDeformation[timepointNumber] = self.sstModel.deformation.copy()
            self.latentDeformationAtlasFileName[timepointNumber] = self.sstModel.deformationAtlasFileName
            self.latentMeans[timepointNumber] = self.sstModel.gmm.means.copy()
            self.latentVariances[timepointNumber] = self.sstModel.gmm.variances.copy()
            self.latentMixtureWeights[timepointNumber] = self.sstModel.gmm.mixtureWeights.copy()

        if self.initializeLatentDeformationToZero:
            for timepointNumber in self.timepointNumberUpdate:
                self.timepointModels[timepointNumber].deformation = self.latentDeformation[timepointNumber].copy()
                self.timepointModels[timepointNumber].latentDeformationAtlasFileName = self.latentDeformationAtlasFileName[timepointNumber].copy()
        ...
        self.timerFit(f"Fit Initalization ")
        ...
        while True:
            ...
            self.timerFit(f"Fit Segmentation {iterationNumber}")
            ...
            for timepointNumber in self.timepointNumberUpdate:
                ...
                timepointPositions = []
                TimepointForLatentNumbers = []

                for neighbouringTimepoint in range(timepointNumber-self.n_neighbors, timepointNumber + self.n_neighbors + 1):
                    if neighbouringTimepoint in self.timepointNumberProcessed + self.timepointNumberUpdate:
                        TimepointForLatentNumbers.append(neighbouringTimepoint)
                ...
                print(f"Doing heavy weights....:{timePointWeights}")
                ...
                self.timerFit(f"Fit Latent Deformation {iterationNumber} for timepoint {timepointNumber}" )
                self.updateGMMLatentVariables(timepointNumber)
                self.timerFit(f"Fit Latent GMM {iterationNumber} for timepoint {timepointNumber}")
    ...
    def updateGMMLatentVariables(self,latentTimePointNumber):
        ...
        if self.updateLatentMeans:
            ...
            for gaussianNumber in range(numberOfGaussians):
                ...
                for timepointNumber in self.timepointNumberProcessed + self.timepointNumberUpdate:
    ...
    def postProcess(self, saveWarp=False):
        ...
        for timepointNumber in self.timepointNumberUpdate:
            ...
            # This is where the segmentation is saved
    ...
    def loadSubjectSpecificTemplate(self):
        # TODO Check if exists and is usable
        sstDir = os.path.join(self.savePath, 'base')
        sstFileNames = []
    ...
    def constructTimepointModels(self):
        ...
        for timepointNumber in range(self.numberOfTimepoints):
            ...
            self.timepointNumberUpdate = [t for t in range(self.numberOfTimepoints) if self.imageFileNamesList[t] in self.updateFileNamesList]
    ...
    def saveFinalState(self):
        finalState = {
            ...
            "sstOptimizationSummary": self.sstModel.optimizationSummary,
            "sstOptimizationHistory": self.sstModel.optimizationHistory,
            "sstNumberOfGaussiansPerClass": self.sstModel.gmm.numberOfGaussiansPerClass,
            "labels": self.sstModel.modelSpecifications.FreeSurferLabels,
            "names":  self.sstModel.modelSpecifications.names,
            "timepointVolumesInCubicMm": self.timepointVolumesInCubicMm,
            "optimizationSummary": self.optimizationSummary,
            "latentMeans": self.latentMeans.copy(),
            "latentVariances": self.latentVariances.copy(),
            "latentMixtureWeights": self.latentMixtureWeights.copy(),
            "latentDeformation": self.latentDeformation.copy(),
            "latentDeformationAtlasFileName": self.latentDeformationAtlasFileName,
            "imageToImageTransformMatrix" : self.imageToImageTransformMatrix,
            "timepointModel": []
        }
        for timepointNumber in self.timepointNumberUpdate + self.timepointNumberProcessed:
            finaleStateTimePoint = {
            "timepointFileName": self.imageFileNamesList[timepointNumber],
            "timepointMeans": self.timepointModels[timepointNumber].gmm.means.copy(),
            "timepointVariances": self.timepointModels[timepointNumber].gmm.variances.copy(),
            "timepointMixtureWeights": self.timepointModels[timepointNumber].gmm.mixtureWeights.copy(),
            #"timepointBiasFieldCoefficients": self.timepointModels[timepointNumber].biasField.coefficients.copy(),
            "timepointDeformations": self.timepointModels[timepointNumber].deformation,
            # this should be without the final deformation, hence save it before postprocessing
            "timepointDeformationAtlasFileNames": self.timepointModels[timepointNumber].deformationAtlasFileName,
            }
            finalState["timepointModel"].append(finaleStateTimePoint)

        finalStatePath = os.path.join(self.savePath, 'finalState.p')
